...
def evaluate(rows, params, raw, evaluationFunction=entropy):
    ...
    return fastforward(rows, params, raw, evaluationFunction)
...
def fastforward(rows, params, raw, columns, evaluationFunction=entropy):
    ...
    if tpF+fnF > 0 and fnF/(tpF+fnF)>0.8 :
        trueBranch = fastforward(set1F, params, raw, evaluationFunction)
        falseBranch = fastforward(set2F, params, raw, evaluationFunction)
        if len(set1)>0 and len(set2)>0:
        ...
        if gainE > 0:
            trueBranch = fastforward(set1E, params, raw, evaluationFunction)
            falseBranch = fastforward(set2E, params, raw, evaluationFunction)
...
if __name__ == '__main__':
    ...
    graph.write_pdf("iris6.pdf")
    graph.write_png("iris6.png")
